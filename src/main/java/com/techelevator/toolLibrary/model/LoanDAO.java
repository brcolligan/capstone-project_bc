...
public class LoanDAO {
	...
	public void saveLoanItem(Loan newLoan) {
		...
		String insertSQL = "INSERT INTO loan(loan_id, inventory_id, tool_name, loan_start_date, loan_due_date,"
				+ " loan_end_date, user_first_name, user_last_name, user_phone_num, user_license_num) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

		jdbcTemplate.update(insertSQL, newLoan.getLoanId(), newLoan.getInventoryId(), newLoan.getToolLoaned(),
				newLoan.getDateOfLoan(), newLoan.getExpectedReturn(), loanEndDate, newLoan.getFirstName(),
				newLoan.getLastName(), newLoan.getPhoneNumber(), newLoan.getDriversLicense()
		// lateFee, 		// maintenanceFee,		// cleaningFee		
		);
	...
	public void updateReturnedItem(int loanId) {
		...
		String insertSQL = "UPDATE loan SET loan_end_date = ?, late_fee = ?, "
				+ "maintenance_fee = ?, cleaning_fee = ? WHERE loan_id = ?";
		jdbcTemplate.update(insertSQL, loanEndDate, lateFee, maintenanceFee, cleaningFee, loanId);
